...
size_Data = [100 100 100 100 100 100 100 100 100 100];

% Class vectors
data_Num = [repmat(0, 100, 1); repmat(1, 100, 1); ...
    repmat(2, 100, 1); repmat(3, 100, 1); repmat(4, 100, 1); ...
    repmat(5, 100, 1); repmat(6, 100, 1); repmat(7, 100, 1); ...
    repmat(8, 100, 1); repmat(9, 100, 1)];


size_Data_Cum = cumsum(size_Data);
size_Data_Cum = [0 size_Data_Cum];   % Zero for the loop (later)
...
for i = 1:10                               % For 0 to 9
        for j = 1:size_Data(i)
            ...
            input_data(1:size(pos, 1), :, size_Data_Cum(i) + j) = pos;
...
for i = 1:size(deg_Of_Rot, 2)              % Size of rotation vector
    ...
    for j = 1:size(input_data, 3)          % Size of data (3rd dim)
        ...
        R_3D_Rotation = [cos(angle) 0 sin(angle); 0 1 0; -sin(angle) 0 ...
            cos(angle)];

        data_Set_Of_Rot = input_data(1:stroke_Size(j), :, j)*R_3D_Rotation;
        ...
        n_Rot_X_Data_Set(1:size(data_Set_Of_Rot, 1), :, (i - 1)* ...
            ...
            [], 1) - min(data_Set_Of_Rot, [], 1)), ...
            size(data_Set_Of_Rot, 1), 1);
...
for i = 1:size(deg_Of_Rot, 2)              % Size of rotation vector
    ...
    for j = 1:size(input_data, 3)          % Size of data (3rd dim)
        ...
        data_Set_Of_Rot = input_data(1:stroke_Size(j), :, j)* ...
            R_3D_Rotation;
...
for i = 1:size(deg_Of_Rot, 2)              % Size of rotation vector
    ...
    for j = 1:size(input_data, 3)          % Size of data (3rd dim)
        ...
        % Transformation matrix
        R_3D_Rotation =[cos(angle) sin(angle) 0; -sin(angle) cos(angle) ...
            0; 0 0 1];
...
u = 0.20;                                  % Initial value

for m = 1:size(stroke_Size_Com, 2)
    
    slope_val(1:2, m) = [N_data_Com(round(stroke_Size_Com(m)*u), 2, m) ...
        - N_data_Com(1, 2, m); N_data_Com(round(stroke_Size_Com(m)*u), ...
...
for k = 1:size(N_data_Com, 3) 
    for y = 0:step_size:1
        ...
        for q = 1:stroke_Size_Com(k) - 1   % -1 for q + 1
...
train_rate = 0.8;                         % Size of train dataset
...
% Train and Test data
train_data = train_profile(:, x_Train);
test_data = train_profile(:, x_Test);

train_classes = data_Num(x_Train, 1);
test_classes = data_Num(x_Test, 1);

% Fitting knn
classes = knn(train_classes, train_data, test_data, 7);

% Measurements
correct_classes = sum(classes == test_classes)/ ...
    size(test_classes, 1);

avarage_correct_class = mean(correct_classes);
